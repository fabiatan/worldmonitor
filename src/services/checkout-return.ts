...
export function handleCheckoutReturn(): CheckoutReturnResult {
  ...
  // Priority ordering:
  //   1. Dodo success status → success
  //   2. Dodo failed status → failed (with raw status for Sentry)
  //   3. WM marker present as success bridge → success
  //   4. Unknown Dodo status (with params but no known value) → failed
  //      so the user sees an actionable banner instead of silent drop
  //   5. Fallback → none
